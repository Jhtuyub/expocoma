...
namespace EXPOCOMA.Funciones
{
    class funcionesdblocal : modelo.dbconexion
    {

        //dblocal __dblocal = new dblocal();
        public String _TIPObasedatos = "dbcompaq";
        ...
        public DataTable llenar_form(String ___TABLA, int ___IDDATO = 0, String ___CAMPO = null, String ___DATO = null)
        {
            ...
            if ((String.IsNullOrEmpty(___CAMPO) && (String.IsNullOrEmpty(___DATO))))
            {
                if (___IDDATO > 0)
                {
                    _sql = "SELECT * FROM " + ___TABLA + " WHERE id = " + ___IDDATO + " ORDER BY id DESC";
                }
                else
                {
                    _sql = "SELECT * FROM " + ___TABLA + " ORDER BY id DESC";
                }
            }
            else
            {
                _sql = "SELECT * FROM " + ___TABLA + " WHERE " + ___CAMPO + " LIKE '%" + ___DATO + "%' ORDER BY id DESC";
            }
